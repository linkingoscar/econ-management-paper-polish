...
| Local number/citation binding | `check_local_bindings.py` | reordered, relocated, or unmatched protected sentence contexts |
| Citation/BibTeX consistency | `check_citations.py` | missing keys, unused keys, malformed DOI/URL |
| LaTeX structure | `audit_latex.py` | missing packages, duplicate labels, refs, figures, citations, and unbalanced environments |
| Meaning-risk markers | `meaning_audit.py` | changed causal, identification, direction, significance, strength, uncertainty, and scope markers |
...
| Blind Agent benchmark | `build_agentic_review_packet.py`, `run_agentic_judges.py`, `adjudicate_agentic_benchmark.py` | anonymous candidates, isolated provenance, hard-gate precedence, confidence mode |
| Adversarial regression | `generate_adversarial_mutations.py` | deterministic operators, safe controls, independent rejection oracle |
